...
static void glnvg__getUniforms(GLNVGshader* shader)
{
	...
	shader->loc[GLNVG_LOC_TRANSFORM] = glGetUniformLocation(shader->prog, "transform");
#if NANOVG_GL_USE_UNIFORMBUFFER
...
static char* loadFileIntoStr(const char* filename)
{
...
//#define NANOVG_GL_USE_SHADER_FILE
#ifndef NANOVG_GL_USE_SHADER_FILE
	static const char* fillVertShader =
		...
		"	ftcoord = tcoord;\n"
		"	fpos = vertex;\n"
		"	vec4 ndc = vec4(2.0*vertex.x/viewSize.x - 1.0, 1.0 - 2.0*vertex.y/viewSize.y, 0, 1);\n"
		"	gl_Position = transform * ndc;\n"
        "   vPos = gl_Position;\n"
		"}\n";
	...
	char* fillVertShader = loadFileIntoStr("shader/nanovg.vert");
	char* fillFragShader = loadFileIntoStr("shader/nanovg.frag");
...
#ifdef NANOVG_GL_USE_SHADER_FILE
...
#ifdef NANOVG_GL_ENABLE_CULL_FACE
...
static void glnvg__renderFlush(void* uptr)
{
	...
	if (gl->ncalls > 0) {
		...
		for (i = 0; i < gl->ncalls; i++) {
			...
			glUniformMatrix4fv(gl->shader.loc[GLNVG_LOC_TRANSFORM], 1, GL_FALSE, call->transform);
...
static void glnvg__renderFill(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation, NVGscissor* scissor, float fringe,
                              const float* bounds, const NVGpath* paths, int npaths, const float* transform)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	GLNVGcall* call = glnvg__allocCall(gl);
	NVGvertex* quad;
	GLNVGfragUniforms* frag;
	int i, maxverts, offset, head;
...
static void glnvg__renderStroke(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation, NVGscissor* scissor, float fringe,
                                float strokeWidth, const NVGpath* paths, int npaths, const float* transform)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	GLNVGcall* call = glnvg__allocCall(gl);
	int i, maxverts, offset, head;
...
static void glnvg__renderTriangles(void* uptr, NVGpaint* paint, NVGcompositeOperationState compositeOperation, NVGscissor* scissor,
                                   const NVGvertex* verts, int nverts, const float* transform)
{
	GLNVGcontext* gl = (GLNVGcontext*)uptr;
	GLNVGcall* call = glnvg__allocCall(gl);
	GLNVGfragUniforms* frag;

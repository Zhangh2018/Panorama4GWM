...
typedef struct PcReadData
{
	...
	int TimeStampex[3]; 			//TimeStampex[0]：time stampex   TimeStampex[1]：time stampey  TimeStampex[2]：time stampey
	...
	short int travel_parking_rect_point0_x;	//行程虚拟车位角点坐标X1
	short int travel_parking_rect_point0_y;	//行程虚拟车位角点坐标Y1
	short int travel_parking_rect_point1_x;	//行程虚拟车位角点坐标X2
	short int travel_parking_rect_point1_y;	//行程虚拟车位角点坐标Y2
	short int travel_parking_rect_point2_x;	//行程虚拟车位角点坐标X3
	short int travel_parking_rect_point2_y;	//行程虚拟车位角点坐标Y3
	short int travel_parking_rect_point3_x;	//行程虚拟车位角点坐标X4
	short int travel_parking_rect_point3_y;	//行程虚拟车位角点坐标Y4

	unsigned char progress_bar;			//进度条进度 
	unsigned char total_park_step;		//泊车总步骤(把数)
	unsigned char current_park_step;	//当前步骤(把数)
	unsigned char optional_parking_status; //1 自选车位可泊  2自选车位不可泊  0 默认
}PCREADDATA;
...
typedef struct
{
    unsigned char Mem_FrontCam[1280*960*2];
	unsigned char Mem_BackCam[1280*960*2];
	unsigned char Mem_LeftCam[1280*960*2];
	unsigned char Mem_RightCam[1280*960*2];
...
{		
	unsigned int TimeStampex_Couter;
	int TimeStampex[3]; 			//TimeStampex[0]：time stampex   TimeStampex[1]：time stampey  TimeStampex[2]：time stampez

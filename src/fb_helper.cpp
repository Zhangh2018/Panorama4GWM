...
unsigned char carlogo[748*332*4];
...
void *Gui_meg_thread(void *t) 
{
	...
	//for(x=0; x<360; x++)
		...
		//}
	//for(i=404;i<272+404;i++)
	//{ 
	//		memcpy(view_mem+i*1920*4+(256+485)*4,carlogo+(i-404)*693*4,693*4);
	//}
	...
	for(i=404-30;i<272+404+30;i++)
	{ 
			memcpy(view_mem_temp+i*1920*4+(256+485)*4,carlogo+(i-404+30)*748*4,748*4);
	}
	...
	{	
		...
		//else
		//{
		//	memset((char*)fb_ptr[0],0,1920*1080*4);
		//}
...
void readbmp()
{
	FILE* fp;
	int i,j;
	if((fp = fopen("/data/carlogo_748x332.bmp", "rb")) == NULL)
	{
		printf("Fail to fopen carlogo_748x332.bmp \n");
	}	
	else
	{	
		fseek(fp,54L,SEEK_SET);
		fread((unsigned char*)carlogo, sizeof(unsigned char), 748*332*4, fp);
		fflush(fp);		
		fclose(fp);	
		for(i=0;i<332;i++)		
		{			
			for(j=0;j<748;j++)			
			{				
				if(*((unsigned int*)carlogo+j+i*748) != 0x00ffffff)
					*((unsigned int*)carlogo+j+i*748) = (*((unsigned int*)carlogo+j+i*748)) |0x60000000;
			}
		}
	}
	#if 0

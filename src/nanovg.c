...
struct NVGstate {
	...
	int fontId;
	float transform[16];
	NVGVertexTransformCallback transformCallback;
	void* transformCallbackCtx;
};
...
void nvgTransform4(NVGcontext* ctx, const float* m)
{
	NVGstate* state = nvg__getState(ctx);
	memcpy(state->transform, m, sizeof(float) * 16);
}

void nvgVertexTransformCallback(NVGcontext* ctx, NVGVertexTransformCallback cb, void* cbCtx)
{
	NVGstate* state = nvg__getState(ctx);
	state->transformCallback = cb;
	state->transformCallbackCtx = cbCtx;
}
...
void nvgReset(NVGcontext* ctx)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		for (int j = 0; j < 4; ++j)
		{
			state->transform[i * 4 + j] = (i == j ? 1 : 0);
		}
	}

	state->transformCallback = NULL;
	state->transformCallbackCtx = NULL;
}
...
static void transformVertexs(NVGvertex* verts, int n, NVGVertexTransformCallback transFunc, void* transCtx)
{
	if (verts != NULL && n > 0 && transFunc != NULL && transCtx != NULL)
	{
		for (int i = 0; i < n; ++i)
		{
			//printf("vertex.xy = (%f, %f)\n", verts[i].x, verts[i].y);
			transFunc(transCtx, &verts[i]);
		}
	}
	//printf("=========================================\n");
}

static void transformPaths(NVGpath* paths, int n, NVGVertexTransformCallback transFunc, void* transCtx)
{
	if (paths != NULL)
	{
		for (int i = 0; i < n; ++i)
		{
			...
			if (path->stroke != NULL && path->nstroke > 0)
			{
				transformVertexs(path->stroke, path->nstroke, transFunc, transCtx);
			}
			if (path->fill != NULL && path->nfill > 0)
			{
				transformVertexs(path->fill, path->nfill, transFunc, transCtx);
			}
		}
	}
}

static void transformPathCache(NVGpathCache* cache, NVGVertexTransformCallback transFunc, void* transCtx)
{
	transformPaths(cache->paths, cache->npaths, transFunc, transCtx);
	//transformVertexs(cache->verts, cache->nverts, transFunc, transCtx);

	// update the bounding box
	cache->bounds[0] = cache->bounds[1] = 1e6f;
	cache->bounds[2] = cache->bounds[3] = -1e6f;

	for(int i = 0; i < cache->nverts; ++i)
	{
		NVGvertex* v = cache->verts + i;
		cache->bounds[0] = nvg__minf(cache->bounds[0], v->x);
		cache->bounds[1] = nvg__minf(cache->bounds[1], v->y);
		cache->bounds[2] = nvg__maxf(cache->bounds[2], v->x);
		cache->bounds[3] = nvg__maxf(cache->bounds[3], v->y);
	}
}
...
void nvgFill(NVGcontext* ctx)
{
	...
	transformPathCache(ctx->cache, state->transformCallback, state->transformCallbackCtx);
	ctx->params.renderFill(ctx->params.userPtr, &fillPaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
						   ctx->cache->bounds, ctx->cache->paths, ctx->cache->npaths, state->transform);
...
void nvgStroke(NVGcontext* ctx)
{
	...
	ctx->params.renderStroke(ctx->params.userPtr, &strokePaint, state->compositeOperation, &state->scissor, ctx->fringeWidth,
							 strokeWidth, ctx->cache->paths, ctx->cache->npaths, state->transform);

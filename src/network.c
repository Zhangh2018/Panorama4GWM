    ...
    while (1) //
    {
		...
		if (networkstatus == 1)
		{
	    	...
	    	if(GetNewPicFrame==1)
	    	{
				...
				GetNewPicFrame=0;
     			memcpy(CAPTURE_MEM, buf_camfront, 1280*960*2);
				memcpy(&CAPTURE_MEM[1280*960*2],buf_camback,1280*960*2);
     			memcpy(&CAPTURE_MEM[1280*960*4],buf_camleft,1280*960*2);
     			memcpy(&CAPTURE_MEM[1280*960*6],buf_camright,1280*960*2);
	    	}
			/*
